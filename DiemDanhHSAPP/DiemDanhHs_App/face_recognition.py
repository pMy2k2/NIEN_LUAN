...
class Face_Recognition:
    ...
    def __init__(self,root):
        ...
        heading = Label(bg_img, text="Hệ thống điểm danh khuôn mặt", font=("Time new Roman", 20, "bold"), bg="white",
                        fg="red2",
        ...
        if(value_from_home=="0" or value_from_home==None):#đăng nhập admin (nếu value_from_home =0 hoặc none thì coi như đăng nhập với quyền admin)
            ...
            for i in subject_ls:
                print(i)
                t = str(i).replace("'", "", 4).replace("(", "").replace(")", "").replace(" ","")  ##loại bỏ các ký tự thừa để hiển thị tên môn,buổi học
        ...
        Left_frame.place(x=30, y=70, width=780, height=540)#vị trí của frame
