...
mydataNotInAtt=[]#mảng dữ liệu sinh viên
# ko điểm danh
class Report:
    def __init__(self,root):
        ...
        #===========variable============
        self.student = StringVar()#biến student để đếm số sinh viên
        # trong database sau khi truy vấn
        self.att=StringVar()#số bản điểm danh
        self.late=StringVar()#số lần đi muộn của các sinh viên

        self.noatt=StringVar()# số lần vắng của các sinh viên
        ...
        student_text=Label(student_frame,text="Số sinh viên"
                                              "",font=("times new roman", 18, "bold"),fg="white",bg="#27a9e3")
        ...
        self.late_group=LabelFrame(Left_frame,bd=1,bg="white",text="Sinh viên"
                                                                   " đi muộn",font=("times new roman", 11, "bold"),fg="black",relief=RIDGE)
        ...
        search_combo = ttk.Combobox(self.late_group, font=("times new roman", 11, "bold"),
                                    ...
                                    state="read only",
                                    width=11)
        search_combo["values"] = ("ID sinh viên"
                                  "", "Ngày","ID Buổi học","Lớp")
        ...
        #===========under-left===============
        #danh sách sinh viên
        # vắng= có điểm danh nhưng đi quá muộn
        self.noatt_group = LabelFrame(Left_frame, bd=1, bg="white", text="Sinh viên"
                                                                         " vắng",
        ...
        search_combo1 = ttk.Combobox(self.noatt_group, font=("times new roman", 10, "bold"),
                                    ...
                                    state="read only",
                                    width=12)
        search_combo1["values"] = ("ID sinh viên"
                                   "", "Ngày","ID Buổi học","Lớp")
        ...
        noatt_lbl = Label(Right_frame, bd=0, bg="white", text="Sinh viên"
                                                              " không điểm danh", font=("times new roman", 12, "bold"),
                         fg="red2", )
        ...
        search_combo2 = ttk.Combobox(self.notinGroup, font=("times new roman", 12, "bold"),
                                    ...
                                    state="read only",
                                    width=12)
        search_combo2["values"] = ("ID sinh viên"
                                   "", "Ngày", "ID Buổi học","Lớp")
    ...
    def exportCsv(self):
        try:
            ...
            df = pd.DataFrame(mydata,
                              columns=['ID sinh viên'
                                       '', 'Tên sinh viên'
                                           '', 'Lớp', 'Ngày', 'ID Buổi học','Trạng thái'])

            writer = pd.ExcelWriter(
                'C:/Users/HAPPY/PycharmProjects/Nien_luan/DiemDanhHSAPP/DiemDanhHs_App/Attendance_CSV/ds_di-muon_' + str(ts) + '_' + str(
    ...
    def exportUnpresetCsv(self):
        try:
            ...
            writer = pd.ExcelWriter(
                'C:/Users/HAPPY/PycharmProjects/Nien_luan/DiemDanhHSAPP/DiemDanhHs_App/Attendance_CSV/ds_vang' + str(ts) + '_' + str(
                    self.today) + '.xlsx', engine='xlsxwriter')
    ...
    def exportNotinCsv(self):
        try:
            ...
            if len(mydataNotInAtt)<1:
                messagebox.showerror("Không có dữ liệu","Không có dữ liệu để xuất file",parent=self.root)
                return  False
            df = pd.DataFrame(mydataNotInAtt,columns= ['ID sinh viên'
                                                       '', 'Tên sinh viên'
                                                           '','Lớp','Ngày','Lớp học','ID Buổi học'])

            writer=pd.ExcelWriter('C:/Users/HAPPY/PycharmProjects/Nien_luan/DiemDanhHSAPP/DiemDanhHs_App/Attendance_CSV/khong_diem_danh_'+str(ts)+'_'+str(self.today)+'.xlsx',engine='xlsxwriter')
