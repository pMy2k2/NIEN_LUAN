...
class Attendance:
    ...
    def __init__(self,root):
        ...
        self.var_atten_idsv = StringVar()#id sinh viên
        self.var_atten_name = StringVar() #tên sinh viên
        ...
        label_Update_att.place(x=50, y=480, width=220, height=35)
        # xuất ra file csv

        update_btn = Button(Left_frame, text="Xuất file CSV", command=self.exportCsv,
                            font=("times new roman", 12, "bold"), bd=0, bg="white", cursor='hand2',
                            activebackground='white',
                            width=220, image=self.photobtn2, fg="white", compound="center")
        update_btn.place(x=50, y=435, width=220, height=35)
        #nút xóa ảnh
        update_btn = Button(Left_frame, text="Xóa", command=self.delete_data,
                            font=("times new roman", 12, "bold"),
                            bd=0, bg="white", cursor='hand2', activebackground='white', width=220, image=self.photobtn2, fg="white", compound="center")
        update_btn.place(x=50, y=530,width=220,height=35 )
    ...
    def exportCsv(self):
        try:
            ...
            #tạo dataframe có các cột : mã điểm danh,Id,tên sinh viên,....
            df = pd.DataFrame(mydata,
                              columns=['Mã Điểm danh', 'ID sinh viên', 'Tên sinh viên', 'Lớp', 'Giờ vào', 'Giờ ra', 'Ngày', 'ID Buổi học', 'Trạng thái'])

            #file excel để xuất dữ liệu
            writer = pd.ExcelWriter(
                'C:/Users/HAPPY/PycharmProjects/Nien_luan/DiemDanhHSAPP/DiemDanhHs_App/Attendance_CSV/ds_diem-danh_'
                + str(ts) + '_' + str(
    ...
    def get_cursor(self,event=""):#hàm sự kiện click vào bảng
        ...
        self.var_atten_id.set(rows[0])#set giá trị mã sinh viên là vị trí 0 của mảng rows
    ...
    def openImage(self):#hàm mở ảnh đã điểm danh
        if self.var_atten_id == "":
            ...
            if not os.path.exists("DiemDanhImage\ " + self.var_atten_id.get() +"Ra" +".jpg"):#nếu ko có ảnh điểm danh ra:
                ...
                cv2.imshow("Ảnh ra khỏi lớp", img)#hiển thị ảnh
            ...
            cv2.imshow("Ảnh ra-vào lớp", Hori)#hiển thị ảnh
        else:
            messagebox.showerror("Lỗi", "Không tìm thấy ảnh điểm danh", parent=self.root)
