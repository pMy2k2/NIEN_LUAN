...
class Lesson:
    def __init__(self,root):
        ...
        classLabel_entry = ttk.Combobox(left_inside_frame, width=22, textvariable=self.var_class,
                                     font=("times new roman", 11, "bold"))
        classLabel_entry["values"] = class_array
        classLabel_entry.current(0)
        ...
        self.AttendanceReportTable = ttk.Treeview(table_frame, column=("id", "timestart", "timeend", "date", "class"),
    ...
    #================Function===================
    def timestart(self, event):#Hàm  tự động chèn dấu hai chấm (:)
    ...
    def add_data(self):
        ...
        time_start_str = self.var_timestart.get()
        time_end_str = self.var_timeend.get()

        # Kiểm tra nếu chuỗi thời gian bắt đầu hoặc kết thúc là rỗng
        if time_start_str == "" or time_end_str == "":
            messagebox.showerror("Error", "Vui lòng nhập thời gian bắt đầu và kết thúc", parent=self.root)
            return
        ...
        if self.var_id.get()=="" or self.var_timestart.get()=="" or self.var_timeend.get()=="" or self.dob_entry.get_date().strftime('%d/%m/%Y')=="" or self.var_class.get()=="" :
